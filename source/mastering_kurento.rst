%%%%%%%%%%%%%%%%%
精通 Kurento
%%%%%%%%%%%%%%%%%

Kurento Architecture
====================

.. toctree::
   :maxdepth: 1

   Kurento Architecture <mastering/kurento_architecture>

Kurento API Reference
=====================

.. toctree::
   :maxdepth: 1

   Kurento API Reference <mastering/kurento_API>

Kurento Protocol
================

.. toctree::
   :maxdepth: 1

   Kurento Protocol <mastering/kurento_protocol>

Advanced Installation Guide
===========================

.. toctree::
   :maxdepth: 1

   Advanced Installation Guide <mastering/advanced_installation_guide>

Working with Nightly Builds
===========================

.. toctree::
   :maxdepth: 1

   Working with nightly builds <mastering/kurento_development>

Kurento Modules
===============

.. toctree::
   :maxdepth: 1

   Kurento Modules <mastering/kurento_modules>

WebRTC Statistics
=================

.. toctree::
   :maxdepth: 1

   WebRTC Statistics <mastering/webrtc_statistics>

Kurento Utils JS
================

.. toctree::
   :maxdepth: 1

   Kurento Utils JS <mastering/kurento_utils_js>

Kurento Java Client JavaDoc
===========================

* `kurento-client-java <./_static/langdoc/javadoc/index.html>`_

Kurento JavaScript Client JsDoc
===============================

* `kurento-client-js <./_static/langdoc/jsdoc/kurento-client-js/index.html>`_

Kurento JavaScript Utils JsDoc
==============================

* `kurento-utils-js <./_static/langdoc/jsdoc/kurento-utils-js/index.html>`_


Securing Kurento Applications
=============================

.. toctree::
   :maxdepth: 1

   Securing Kurento Applications <mastering/securing-kurento-applications>